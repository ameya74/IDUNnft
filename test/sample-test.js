...
describe("NFT", function () {
  it("Should mint a NFT and Token and also put it on the marketPlace ", async function () {
    ...
    //Test to add address to the whitelist
    const AddUser = await nft.addUser(addr1.address);
    await AddUser.wait();
    
    const verified = await nft.verifyUser(addr1.address);
